...
class vtk_makeRectilinearGrid():
    ...
    def vtk_add_RectilinearGridTag( self, Data=None, Axis=None, DataName=None, VectorData=False, DataDims=None, WholeExtent=None, \
        ...
        # ------------------------------------------------- #
        self.vtkContents  = self.vtkContents + '<RectilinearGrid WholeExtent="{0}">\n'.format( WholeExtent )
        ...
        # ------------------------------------------------- #
        self.vtkEndTags   = '</RectilinearGrid>\n' + self.vtkEndTags
    ...
    def vtk_writeFile( self, vtkFile=None ):
        if ( vtkFile is not None ): self.vtkFile = "out.vtr"
        with open( self.vtkFile, "w" ) as f:
            f.write( self.vtkContents )
            f.write( self.vtkEndTags  )
        subprocess.call( ( "xmllint --format --encode utf-8 {0} -o {0}".format( self.vtkFile ) ).split() )
